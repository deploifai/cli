...
class AWSDataStorageHandler:
  ...
  def upload_file(self, file: Path, directory, container_name, pbar):
    self.client.upload_file(str(file), container_name, str(file.relative_to(directory)))
    pbar.update(1)
  ...
  def upload_dataset(self, directory: Path, container_name):
    ...
    with tqdm(total=len(files)) as pbar:
      with ThreadPoolExecutor(max_workers=5) as ex:
        futures = [ex.submit(self.upload_file, file_path, directory, container_name, pbar=pbar) for file_path in files]
