...
def add_storage_configs(storage_id, containers, context):
    ...
    data_storage_config = {"id": storage_id, "provider": cloud_provider}
    if cloud_provider == "AZURE":
        data_storage_config["storage"] = storage_details["cloudProviderYodaConfig"][
            "azureConfig"
        ]["storageAccount"]
    ...
    if yaml_config_dict is not None:
        if yaml_config_dict.get("datastorage"):
            raise DeploifaiDataAlreadyInitialisedError(
                "This folder already has a storage attached to it."
            )
    else:
        yaml_config_dict = {}
